# -*- coding: utf-8 -*-
from _entorno import conectarNao, conectarInternet
...
sintomas={}
sintomas["relacionado a tos"] = ['tos','tos seca','tos seca persistente',\
                'tos crónica','tos con sangre','tos sibilante','tos amarilla','tos verdosa',\
...
opciones_sintomas = ["no", "repetir", "ya no tengo más síntomas"]
admitidos_sgenerales = sgenerales + opciones_sintomas
...
scovid = ["tos","tos seca","tos seca persistente","tos cronica","tos con sangre","nariz que moquea",\
    "dificultad para respirar","sensación de opresión en el pecho","dolor de pecho","dolor agudo en el pecho","falta de aliento",\
    "dolor de cabeza","dolores musculares","dolor de garganta",\
...
opciones_celular = ["me equivoqué","confirmo"]
admitidos_celular = digitos+confirmacion+opciones_celular
...
def obtenerDatosPre():
    ...
    while askNombre:
        ...
        time.sleep(1)
        tts_service.say("¿Cuál es su nombre estimado paciente?")
    ...
    while askSexo:
        ...
        time.sleep(1)
        tts_service.say("¿Cuál es su sexo biológico? Femenino. O. Masculino.")
    ...
    while askSGeneral:
        ...
        sr_service.pause(True)
        sr_service.setVocabulary(admitidos_sgenerales,True)
        ...
        if(decir_sgenerales):
            time.sleep(1)
            tts_service.say("Elija alguna de las siguientes categorías "+lista_sgenerales)
        ...
        if (sgeneral_pal in admitidos_sgenerales) and (sgeneral_prob >= prob_aceptable):
            ...
            admitidos_sespecificos = sintomas[sgeneral_pal] + opciones_sintomas
            ...
            while askSEspecifico:
                ...
                sr_service.pause(True)
                sr_service.setVocabulary(admitidos_sespecificos,True)
                ...
                if(decir_sespecificos):
                    time.sleep(1)
                    tts_service.say("Repita alguno de los siguientes síntomas "+lista_sespecificos)
                ...
                time.sleep(1)
                tts_service.say("Diga repetir para mencionar los síntomas nuevamente. Diga no para cambiar de categoría. Diga ya no tengo más síntomas para empezar a procesar.")
                ...
                if (sespecifico_pal in admitidos_sespecificos) and (sespecifico_prob >= prob_aceptable):
                    print("sí escuche")
                    askConfirmacion = True
                            ...
                            if confirmado_pal == "si":
                                ...
                                if contar_scovid >= nummax_scovid: #si supera el límite d síntomas similares de covid
                                    ...
                                    while askPruebaCovid:
                                        ...
                                        time.sleep(1)
                                        tts_service.say("Usted presenta por lo menos "+ str(nummax_scovid) +" síntomas de Covid. ¿Se ha realizado la prueba de descarte de Covid?")
                                        ...
                                        if (pruebaCovid_pal in confirmacion) and (pruebaCovid_prob >= prob_aceptable):
                                            ...
                                            if pruebaCovid_pal == "no":
                                                time.sleep(1)
                                                tts_service.say("Por favor, realícese una prueba de descarte. Hasta pronto")
    ...
    time.sleep(1)
    tts_service.say("Paciente masculino") 
    edad_save = 21
    time.sleep(1)
    tts_service.say(str(edad_save) + " años")
    ...
    return sexo_save, edad_save, mis_sespecificos, mis_intensidades

fsexo, fedad, fsespecificos, fintensidades = obtenerDatosPre()

def procesarDatos(edad_save,sexo_save,mis_sespecificos,mis_intensidades):
...
procesarDatos(fsexo, fedad, fsespecificos, fintensidades)
...
def obtenerDatosPost():
    ...
    #!------------------------------------CELULAR-------------------------------------
    print("len(admitidos_celular): "+str(len(admitidos_celular)))
    sr_service.pause(True)
    sr_service.setVocabulary(admitidos_celular,True)

    time.sleep(1)
    tts_service.say("Por favor, dícteme un número celular de 9 dígitos para enviar los resultados. Diga Me equivoqué para volver a empezar.") 
    ...
    while askCel:
        ...
        if (num_pal in admitidos_celular) and (num_prob >= prob_aceptable):
            ...
            if len(cel) == 9:
                ...
                time.sleep(1)
                tts_service.say("El número recibido es ")
                for c in cel:
                    time.sleep(1)
                    tts_service.say(c)
                ...
                time.sleep(1)
                tts_service.say("Diga confirmo para enviar el mensaje o no diga nada para volver a empezar")
                ...
                if (confirmado_pal == "confirmo") and (confirmado_prob >= prob_aceptable):
            ...
            if len(cel) > 9:
                cel = "" #volver a empezar
                time.sleep(1)
                tts_service.say("El número celular tiene más de 9 dígitos. Volvamos a empezar")
        else:
            time.sleep(1)
